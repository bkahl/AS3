		...
		public function TweenLiteVars($vars:Object = null) {
			...
			if ($vars != null) {
				for (var p:String in $vars) {
					if (p == "blurFilter" || p == "glowFilter" || p == "colorMatrixFilter" || p == "bevelFilter" || p == "dropShadowFilter") {
						//ignore filters - they must be handled differently in the TweenFilterLiteVars subclass
					} else {
						this[p] = $vars[p];
					}
				}
